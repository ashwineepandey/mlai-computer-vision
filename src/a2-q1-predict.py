...
@timer
def save_confusion_matrix(conf, cm, dataset, loss, accuracy):
    """
    ...
    # save figure to file
    fig = plot_confusion_matrix(cm, classes, normalize=False, loss=loss, accuracy=accuracy)
...
def main():
    ...
    # load model
    model = load_best_model(conf.paths.a2_q1_model, 
                            conf.a2_q1_hyperparams.learning_rate[0], 
                            conf.a2_q1_hyperparams.momentum[0])
    # evaluate model on test set
    loss, acc = evaluate_model(model, data['testX'], data['testY'])
    predY, testY = predict(model, data['testX'], data['testY'])
    cm = confusion_matrix(testY, predY)
    save_confusion_matrix(conf, cm, dataset='test', loss=loss, accuracy=acc)
